...
def main():
    ...
    # WORKAROUND: Manually fix Sources section if empty
    if "**Articles:**\n\n\n**Reddit" in report or "**Articles:**\n\n**Reddit" in report:
        print("  → Fixing empty Sources section...")
        sources_content = "\n## Sources\n\n**Articles:**\n\n"
        for i, article in enumerate(collected_data['rss_articles'][:15], 1):
            sources_content += f"- [{article['title']}]({article['url']}) - {article['source']} ({article['published']})\n"
        sources_content += "\n**Reddit Discussions:**\n\n"
        for i, post in enumerate(collected_data['reddit_posts'][:15], 1):
            subreddit = post.get('subreddit', {})
            if isinstance(subreddit, dict):
                subreddit_name = subreddit.get('name', 'unknown')
            else:
                subreddit_name = subreddit
            sources_content += f"- [r/{subreddit_name} - {post['title']}]({post['url']}) (Score: {post['score']})\n"
        sources_content += f"\n---\n\n*Generated by PM Radar • {date_str}*\n"

        # Replace empty sources
        if "## Sources" in report:
            start = report.index("## Sources")
            report = report[:start] + sources_content
            with open(report_path, 'w') as f:
                f.write(report)
