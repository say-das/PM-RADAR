"""
Competitor Changelog Scraper v3
Uses Scrapling (adaptive scraper with stealth) + GPT-4o for extraction
Handles JS-rendered pages with built-in anti-bot bypass
"""
...
class ChangelogScraperV3:
    """
    Scraper using Scrapling for adaptive, stealthy scraping + GPT-4o extraction.
    Automatically handles JS rendering and bypasses anti-bot protections.
    """
    ...
    def _fetch_with_scrapling(self, url):
        ...
        try:
            ...
            # Fetch with stealth (bypasses Cloudflare, renders JS)
            ...
            raise Exception(f"Scrapling error: {str(e)[:100]}")

    def fetch_and_extract(self, url, competitor_name, product_name, scrape_mode="keyword_filter"):
        """Fetch HTML and extract changes using Scrapling + GPT-4o"""

        mode_label = "FULL" if scrape_mode == "full" else "FILTERED"
        print(f"  [{product_name}] ({mode_label} via Scrapling)")
        ...
        try:
            print(f"    → Scraping with Scrapling (stealth mode)...")
            text_content = self._fetch_with_scrapling(url)

            # Truncate to save tokens
            max_tokens = self.config['extraction_settings']['max_tokens_per_page']
            truncated = text_content[:max_tokens * 4]
...
"""
    ...
    def collect_all(self):
        ...
        print("\n" + "="*70)
        print("COMPETITOR CHANGELOG COLLECTION (V3 - Scrapling)")
        print("="*70 + "\n")
